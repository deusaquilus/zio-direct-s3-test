        ...
        override def transformStatement(stmt: Statement)(owner: Symbol): Statement = {
          val lifted =
            pf.lift(stmt) match {
              ...
              case Some(other)           => report.errorAndAbort(s"Transformed the term ${other.show} into something that is not a Statement.")
              case None                  => None
            }
        ...
        override def transformTerm(stmt: Term)(owner: Symbol): Term = {
          val lifted =
            pf.lift(stmt) match {
              ...
              case Some(other)      => report.errorAndAbort(s"Transformed the term ${other.show} into something that is not a Term.")
              case None             => None
            }
